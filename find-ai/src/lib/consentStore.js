...
export function buildWhatsAppShareUrl({ requestId, requesterDisplay, propertyAddress, requestedTier, phone = '' }) {
  const tierLabels = {
  ...
  const what = tierLabels[requestedTier] || 'identity info';
  const url = (typeof window !== 'undefined' ? window.location.origin : 'https://veri.ai') + `/consent/${requestId}`;
  const lines = [
    `Hi — ${requesterDisplay || 'a landlord'} on Veri.ai is requesting your ${what}`,
