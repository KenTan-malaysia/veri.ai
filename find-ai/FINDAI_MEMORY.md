...
> **Last updated:** 2026-04-26 (v3.4.28 — Anonymous-by-default + 5-tier identity reveal + Agent as third user type LOCKED. New docs: `ARCH_REVEAL_TIERS.md` (the reveal model + state machine + PDPA implications) + `ARCH_AGENT_PROFILE.md` (agent profile + gatekeeper role + BOVAEP-anchored verification). `ARCH_USER_PROFILES.md` updated to three-sided platform. · Cakap 2.0 · DNA: TRUST BEFORE SIGNING).
...
**Anonymous-by-default + 5-tier reveal (v3.4.28):** Tenant identity hidden from landlord by default. Trust Card shows Trust Score + categorical attributes only. Reveal happens in 5 tiers (T0 anonymous → T1 categorical → T2 first name → T3 meeting identity → T4 contact reveal → T5 full PII at signing), gated by tenant consent + agent action (where present) + audit log. See `ARCH_REVEAL_TIERS.md`.

**Agent as third user type (v3.4.28):** Property agent is now a first-class user with their own profile, dashboard, and gatekeeper role over reveal tiers. BOVAEP-anchored verification (REN/REA/PEA registration). Agent never sees the actual Trust Score (only submitted/not-submitted + tier state). Premium agent tier (RM 200-500/mo, post-30k) adds co-branded Trust Cards + bulk management. Agency tier (RM 2-10k/mo) adds multi-agent + white-label. See `ARCH_AGENT_PROFILE.md`.
...
**For Zeus sessions: REJECT any feature that exposes tenant identity by default.** Trust Card defaults to T0 (anonymous). Identity reveal is gated through `ARCH_REVEAL_TIERS.md`. If a request asks for "show name to landlord upfront" or "skip the consent flow," push back and reference the doc. Anonymous-by-default is structural — agent moat, anti-discrimination, tenant safety, marketplace differentiation all depend on it.
...
| `ARCH_AGENT_PROFILE.md` | Agent as third user type + gatekeeper role + BOVAEP verification + dashboard + premium tiers | v3.4.28 |
...
| **Anonymous-by-default Trust Card.** | Locked v3.4.28. Tenant identity hidden from landlord by default. Reveal in 5 tiers (T0→T5) gated by consent + agent action + audit log. Reject any feature that exposes name/IC/contact by default. See `ARCH_REVEAL_TIERS.md`. |
...
├── ARCH_AGENT_PROFILE.md         Agent as third user type + gatekeeper (v3.4.28)
...
3. **Greet:** *"Resuming Find.ai v3.4.28 — major model shift locked: Find.ai is now anonymous-by-default with 5-tier identity reveal (T0→T5), Agent is a first-class user type with gatekeeper authority, PDPA infrastructure required (Find.ai is a data controller now). Three new docs: `ARCH_REVEAL_TIERS.md`, `ARCH_AGENT_PROFILE.md`, updated `ARCH_USER_PROFILES.md`. TOOL 1 v0 mock still runs the old flow (shows tenant name) — needs refactor for anonymous default. Sprint 1 (viral mechanic — `/trust/[reportId]`) is now THE foundation, blocking everything. Lawyer engagement upgraded from overdue to BLOCKING (PDPA data controller status). Reminder: Anthropic credits still need adding at console.anthropic.com/settings/billing. Want to ship Sprint 1 (`/trust/[reportId]` + anonymous Trust Card render), refactor TOOL 1 data model for tier-gated PII, kick off lawyer engagement, or push v3.4.28 to git first?"*
