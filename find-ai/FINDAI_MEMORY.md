...
> **Last updated:** 2026-04-26 (v3.4.32 — Intake Form direction LOCKED in doctrine, build deferred to Sprint 4. Find.ai expands from "trust before signing" to "trust before introducing." 4-category field principle (A/B/C/D) locks the field-by-field decisions. Race/religion = opt-in with discrimination warning + audit log (Option B). Specific employer/income = categorical at intake, opt-in specific reveal at T4. Pilot research questions added to validate before build. · Cakap 2.0 · DNA: TRUST BEFORE SIGNING).
...
| **Intake form is upstream of screening, 4-category field principle.** | Locked v3.4.32 (build Sprint 4). Find.ai owns pre-screening intake too. Fields: A) Always-required operational (pax, budget, dates, etc.), B) Optional operational (job location, nationality, furniture), C) Opt-in with discrimination warning (race, religion, family composition), D) Categorical-only never specific (employer category, income range). Specific employer/exact income/IC are NEVER on intake — only at higher reveal tiers. See `ARCH_INTAKE_FORM.md`. |
